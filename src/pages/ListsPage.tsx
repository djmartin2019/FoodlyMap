...
export default function ListsPage() {
  ...
  const handleCreateList = async (e: FormEvent) => {
    ...
    if (trimmedName.length > MAX_LIST_NAME_LENGTH) {
      setError(`List name must be ${MAX_LIST_NAME_LENGTH} characters or less`);
      return;
    }
    ...
    try {
      const { data, error: createError } = await supabase
        ...
        .insert([
          {
            owner_id: user.id,
            name: trimmedName.slice(0, MAX_LIST_NAME_LENGTH),
            description: trimmedDescription ? trimmedDescription.slice(0, MAX_DESCRIPTION_LENGTH) : null,
          ...
          <form onSubmit={handleCreateList} className="mb-6 rounded-lg border border-surface/60 bg-surface/30 p-6 sm:mb-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                {formData.name.length > MAX_LIST_NAME_LENGTH * 0.9 && (
                  <p className="mt-1 text-xs text-text/50">
                    {formData.name.length}/{MAX_LIST_NAME_LENGTH} characters
                  </p>
                )}
              </div>
              <div>
